...
//<2018/09/19,Yuchen-[4101] Parsing calibration data
#include <linux/kernel.h>
#include <linux/unistd.h>
#include <linux/file.h>
#include <linux/mm.h>
#define  ps_uncover_path 		"/persist/sensors/ps_uncover"
#define  ps_cover_path 			"/persist/sensors/ps_cover"
#define  als_lux500_path 		"/persist/sensors/lsensor"
//>2018/09/19,Yuchen
...
//#define STK_CHK_REG
//#define STK_GES
#define QUALCOMM_PLATFORM /* 2018/09/11,Yuchen reserved */
...
#ifdef STK_TUNE0
	//<2019/07/31,Yuchen-[4101] set pre-MP golden value
	///#define STK_MAX_MIN_DIFF	200
	///#define STK_LT_N_CT	100 /* STK_HT_N_CT - 50 */
	///#define STK_HT_N_CT	150 /* 3cm - uncover */
	#define STK_MAX_MIN_DIFF	600 /* set the value around 1cm~2cm */
	#define STK_LT_N_CT	320 /* 4cm delta = 3cm delta * 0.8 */
	#define STK_HT_N_CT	400 /* 3cm delta */
	//>2019/07/31,Yuchen
...
#ifdef QUALCOMM_PLATFORM
//Yuchen	#define ALS_NAME		"stk3x1x-ls"
...
//2018/10/22,Yuchen -[4101] P/L Sensor DVT1 calibration.
//                   alsctrl_reg = 0x39 -> 0x38
//                   transmittance = 500 -> 1100
static struct stk3x1x_platform_data stk3x1x_pfdata={
  ...
  .alsctrl_reg = 0x38, 	/* als_persistance=1, als_gain=64X, ALS_IT=100ms */
...
static bool is_read_ps_data=false;//2018/09/19,Yuchen-[4101] Parsing calibration data
static bool is_read_als_data=false;//2018/09/19,Yuchen-[4101] Parsing calibration data
...
{
    //<2018/09/19,Yuchen-[4101] modify lux formula
    //alscode += ((alscode<<7)+(alscode<<3)+(alscode>>1));
    //alscode<<=3;
    //alscode/=ps_data->als_transmittance;
    alscode = alscode * 500 / ps_data->als_transmittance;
    //>2018/09/19,Yuchen
	return alscode;
}
...
uint32_t stk_lux2alscode(struct stk3x1x_data *ps_data, uint32_t lux)
{
    ...
    //    lux = (1<<16) -1;
    lux = lux * ps_data->als_transmittance / 500;
    //>2018/09/19,Yuchen
    return lux;
}
...
{
	struct file *fp;
//	printk(KERN_ERR "[YC] openFile enter\n");
...
mm_segment_t initKernelEnv(void)
{
    mm_segment_t oldfs;
    oldfs = get_fs();
    set_fs(KERNEL_DS);
...
bool is_chr_number(char c_check)
{
    return (c_check >= 0x30 && c_check <= 0x39 );
}
static uint16_t ArrayChr2Uint16(char arrLow[], const int cnArrLength){
	uint16_t convert_value=0x0;
	unsigned long offset = 0;
	int ret;

	ret = kstrtoul(arrLow, cnArrLength, &offset);
	if(ret < 0){
	...
	convert_value = offset & 0xFFFF;
	return convert_value;
}
int readLowHighLux( char *path, uint16_t *return_value, const int cnArrLength)
{
	int ret = 0;
  	mm_segment_t oldfs;
	struct file *fp;
	...
	if(fp != NULL)
	{
          printk(KERN_ERR "[YC] fp not NULL\n");
          if (fp->f_op && fp->f_op->read){
            memset(arrLow, 0x00, sizeof(arrLow));
            fp->f_op->read(fp, arrLow, cnArrLength, &fp->f_pos);
            convert_value = ArrayChr2Uint16(arrLow, cnArrLength);
            printk(KERN_ERR "[YC] convert_value[%d]\n", convert_value);
          }
          if(convert_value<=0)ret=1;
          else *return_value = convert_value;
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
  int32_t ret;
	...
	uint16_t p_th_low, p_th_high;//2018/09/19,Yuchen
	uint32_t p_cal=0;//2019/07/31,Yuchen
...
#endif
		//<2018/09/19,Yuchen-[4101] Parsing calibration data
		printk(KERN_ERR "[YC] is_read_ps_data:[%s]\n", is_read_ps_data?"TRUE":"FALSE");
//		if(!is_read_ps_data){
			p_th_low=p_th_high=0;
			printk(KERN_ERR "[YC]before. HT=%d, LT=%d\n", ps_data->ps_thd_h, ps_data->ps_thd_l);
			ret = readLowHighLux( ps_uncover_path, &p_th_low, 10);//get uncover file
			if(ret == 0){
				ret = readLowHighLux( ps_cover_path, &p_th_high, 10);//get cover file
			}
			printk(KERN_ERR "[YC] ps ret[%d] - p_th_low[%d] -  p_th_high[%d]\n", ret, p_th_low, p_th_high);
			if(ret==0){
				//<2019/07/31,Yuchen-[4101] modify formulation
				///if( p_th_low!=0)	ps_data->ps_thd_l = p_th_low;
				///if( p_th_high!=0)	ps_data->ps_thd_h = p_th_high;
				if( ((p_th_high-p_th_low) > 200) && ((p_th_high-p_th_low) < 600) ){
					ps_data->ps_thd_h = p_th_high;//3cm raw data
					p_cal = (p_th_high-p_th_low)*8/10+p_th_low;//around 4cm raw data
					ps_data->ps_thd_l = (uint16_t)p_cal;

				  //[hint] above setting only set first time judement
				  //       If you want dync_k also use k value
				  //       you have to modify ps_data->stk_ht_n_ct & ps_data->stk_lt_n_ct
				  //       e.g.
				  //if(!is_read_ps_data){
				  //  ps_data->stk_max_min_diff = 600; //vendor suggest, set the value around 1cm~2cm
				  //  ps_data->stk_ht_n_ct = (p_th_high-p_th_low);//3cm delta
				  //  ps_data->stk_lt_n_ct = (p_th_high-p_th_low)*0.8;//vendor's Sensor IC is not linear, so 4cm_delta's rawdata around 3cm_delta*0.8
				  //}
				}
				//>2019/07/31,Yuchen
				printk(KERN_ERR "[YC]after. HT=%d, LT=%d\n", ps_data->ps_thd_h, ps_data->ps_thd_l);
				is_read_ps_data=true;
			}
//		}
		//>2018/09/19,Yuchen
		stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
  int32_t ret;
	...
	uint16_t l_th_lux;//2018/09/19,Yuchen
	...
	if (enable) {
		//<2018/09/19,Yuchen-[4101] Parsing calibration data
		printk(KERN_ERR "[YC] is_read_als_data:[%s]\n", is_read_als_data?"TRUE":"FALSE");
//		if(!is_read_als_data){
			l_th_lux=0;
			printk(KERN_ERR "[YC]before. luxTran=%d\n", ps_data->als_transmittance);
			ret = readLowHighLux( als_lux500_path, &l_th_lux, 10);//get lux 500 file
			printk(KERN_ERR "[YC] als ret[%d] - l_th_lux[%d] \n", ret, l_th_lux );
			if(ret==0){
				if(l_th_lux!=0)		ps_data->als_transmittance = l_th_lux;
				printk(KERN_ERR "[YC]after. luxTran=%d\n", ps_data->als_transmittance);
				is_read_als_data=true;
			}
//		}
		//>2018/09/19,Yuchen
		ret = stk3x1x_device_ctl(ps_data, enable);
...
static DEVICE_ATTR(ir_code,         0444, stk_als_ir_code_show,         NULL);
#ifdef STK_ALS_FIR
...
#ifdef STK_GES
static DEVICE_ATTR(ges_enable,          0664, stk_ges_enable_show,          stk_ges_enable_store);
static DEVICE_ATTR(ges_code,            0664, stk_ges_code_show,            stk_ges_code_store);
static DEVICE_ATTR(ges_poll,            0664, stk_ges_poll_show,            stk_ges_poll_store);
static DEVICE_ATTR(ges_recv,            0664, stk_recv_show,                stk_recv_store);
static DEVICE_ATTR(ges_send,            0664, stk_send_show,                stk_send_store);

static struct attribute *stk_ges_attrs [] =
{
  &dev_attr_ges_enable.attr,
  &dev_attr_ges_code.attr,
  &dev_attr_ges_poll.attr,
	&dev_attr_ges_recv.attr,
	&dev_attr_ges_send.attr,
  NULL
};
...
static DEVICE_ATTR(ps_enable,          0664,   stk_ps_enable_show,         stk_ps_enable_store);
static DEVICE_ATTR(ps_enable_aso,       0664,   stk_ps_enable_aso_show,     stk_ps_enable_aso_store);
static DEVICE_ATTR(ps_distance,        0664,   stk_ps_distance_show,       stk_ps_distance_store);
static DEVICE_ATTR(ps_offset,          0664,   stk_ps_offset_show,         stk_ps_offset_store);
static DEVICE_ATTR(ps_code,            0444,   stk_ps_code_show,           NULL);
static DEVICE_ATTR(ps_code_thd_l,        0664,   stk_ps_code_thd_l_show,     stk_ps_code_thd_l_store);
static DEVICE_ATTR(ps_code_thd_h,        0664,   stk_ps_code_thd_h_show,     stk_ps_code_thd_h_store);
static DEVICE_ATTR(ps_recv,            0664,   stk_recv_show,              stk_recv_store);
static DEVICE_ATTR(ps_send,            0664,   stk_send_show,              stk_send_store);
static DEVICE_ATTR(ps_all_reg,          0444,   stk_all_reg_show,           NULL);
static DEVICE_ATTR(ps_status,          0444,   stk_status_show,            NULL);
#ifdef STK_TUNE0
static DEVICE_ATTR(ps_cali,            0444,   stk_ps_cali_show,           NULL);
static DEVICE_ATTR(ps_maxdiff,         0664,   stk_ps_maxdiff_show,        stk_ps_maxdiff_store);
static DEVICE_ATTR(ps_ltnct,           0664,   stk_ps_ltnct_show,          stk_ps_ltnct_store);
static DEVICE_ATTR(ps_htnct,           0664,   stk_ps_htnct_show,          stk_ps_htnct_store);
#endif

static struct attribute *stk_ps_attrs [] =
{
  &dev_attr_ps_enable.attr,
  &dev_attr_ps_enable_aso.attr,
...
#if (!defined(STK_POLL_ALS) || !defined(STK_POLL_PS) || defined(QUALCOMM_PLATFORM))/* 2018/09/11,Yuchen error handle */
err_stk3x1x_setup_irq:
#endif/* 2018/09/11,Yuchen error handle */
