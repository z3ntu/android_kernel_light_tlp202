/* drivers/misc/gmc306.c - gmc306 compass driver
 *
...
#ifndef GMC306_VIO_NONE
#define GMC306_VIO_NONE
#endif

#ifndef GMC306_PINCTRL_NONE
#define GMC306_PINCTRL_NONE
#endif
...
#ifdef GMC306_VIO_NONE
//#define AKM09911_VDD_MIN_UV	2000000
#define GMC306_VDD_MIN_UV	1650000
//#define AKM09911_VDD_MAX_UV	3300000
#define GMC306_VDD_MAX_UV	1950000
#else
//#define AKM09911_VDD_MIN_UV	2000000
#define GMC306_VDD_MIN_UV	2000000
//#define AKM09911_VDD_MAX_UV	3300000
#define GMC306_VDD_MAX_UV	3300000
//#define AKM09911_VIO_MIN_UV	1750000
#define GMC306_VIO_MIN_UV	1750000
//#define AKM09911_VIO_MAX_UV	1950000
#define GMC306_VIO_MAX_UV	1950000
#endif
...
//		gpio_set_value(akm->gpio_rstn, 1);
		...
		dev_warn(&akm->i2c->dev,
					"aAKECS_Reset soft reset akm->gpio_rstn=%d\n",
					akm->gpio_rstn);
...
//	case AKM_MODE_CONTINUOUS_100HZ:
	case AKM_MODE_CONTINUOUS_100HZ:
	case AKM_MODE_CONTINUOUS_200HZ:
...
//	err = akm_i2c_rxdata(akm->i2c, buffer, AKM_SENSOR_DATA_SIZE);
	//dev_info(&akm->i2c->dev, "%s before akm_i2c_rxdata", __func__);
	err = akm_i2c_rxdata(akm->i2c, buffer, AKM_SENSOR_DATA_SIZE);
	if (err < 0) {
...
//		dev_dbg(&akm->i2c->dev, "Data over run!\n");
	...
	temp=buffer[3];buffer[3]=buffer[4];buffer[4]=temp;
	temp=buffer[5];buffer[5]=buffer[6];buffer[6]=temp;
...
//		dev_vdbg(&akm->i2c->dev, "IOCTL_RESET called.");
		dev_vdbg(&akm->i2c->dev, "IOCTL_RESET called.");
		dev_warn(&akm->i2c->dev, "IOCTL_RESET called.");
...
//		return AKM_MODE_CONTINUOUS_100HZ;
		...
		return AKM_MODE_CONTINUOUS_200HZ;
}

extern uint32_t hardware_version;
...
//			struct akm_compass_data, cdev);
	...
	dev_warn(&akm->i2c->dev,
				"akm_enable_set enter enabled=%d\n",
				enable);
	dev_err(&akm->i2c->dev, "hardware_version = %d\n", hardware_version);
	...
	struct akm_compass_data *akm, char const *buf, size_t count, int pos)
{
	...
	dev_warn(&akm->i2c->dev,
				"akm_compass_sysfs_enable_store count=%d\n",
				(int)count);
	if (NULL == buf){
		dev_warn(&akm->i2c->dev,
					"akm_compass_sysfs_enable_store NULL == buf =%d\n",
					(int)buf);
		return -EINVAL;
	}
...
//	if (strict_strtol(buf, AKM_BASE_NUM, &en))
	if (kstrtol(buf, AKM_BASE_NUM, &en)){
		dev_warn(&akm->i2c->dev,
					"akm_compass_sysfs_enable_store kstrtol return buf =%d\n",
					(int)buf);
		return -EINVAL;
	}
	...
	en = en ? 1 : 0;
	dev_warn(&akm->i2c->dev,
				"akm_compass_sysfs_enable_store enabled=%d\n",
				(int)en);
...
//	uint8_t rbuf[AKM_SENSOR_DATA_SIZE];
	...
	dev_info(&akm->i2c->dev, "akm_sysfs_bdata_show enter.\n");
//	mutex_lock(&akm->sensor_mutex);
	if (akm_report_data(akm) < 0)
...
//	uint8_t regs[AKM_REGS_SIZE];
	...
	dev_info(&akm->i2c->dev, "akm_sysfs_regs_show enter.\n");
...
//	struct akm_compass_data *akm = handle;
	...
	dev_info(&akm->i2c->dev, "akm_compass_irq enter.\n");
...
//20181204 Frank Cheng commented this for LightPhone E-Compass vdd & I2C's power pin are the same source +
...
//	err = AKECS_SetMode(akm, AK09911_MODE_FUSE_ACCESS);
	err = AKECS_SetMode(akm, GMC306_MODE_FUSE_ACCESS);
	if (err < 0)
		return err;

//	akm->sense_conf[0] = AK09911_FUSE_ASAX;
	akm->sense_conf[0] = GMC306_FUSE_ASAX;
...
//	err = AKECS_SetMode(akm, AK09911_MODE_POWERDOWN);
	err = AKECS_SetMode(akm, GMC306_MODE_POWERDOWN);
	if (err < 0)
		return err;

	/* Check read data */
//	if ((akm->sense_info[0] != AK09911_WIA1_VALUE) ||
	if ((akm->sense_info[0] != GMC306_WIA1_VALUE) ||
//			(akm->sense_info[1] != AK09911_WIA2_VALUE)){
			(akm->sense_info[1] != GMC306_WIA2_VALUE)){
		dev_err(&client->dev,
...
static int akm_compass_power_set(struct akm_compass_data *data, bool on)
{
	...
	dev_warn(&data->i2c->dev,
				"akm_compass_power_set enabled=%d\n",
				(int)on);
	if (!on && data->power_enabled) {
		...
		msleep(80);
		dev_warn(&data->i2c->dev,
				"Power on=%d. from not enable to enabled=%d\n",
...
//				AKM09911_VDD_MIN_UV, AKM09911_VDD_MAX_UV);
				GMC306_VDD_MIN_UV, GMC306_VDD_MAX_UV);
...
//				AKM09911_VIO_MIN_UV, AKM09911_VIO_MAX_UV);
				GMC306_VIO_MIN_UV, GMC306_VIO_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
...
//		regulator_set_voltage(data->vdd, 0, AKM09911_VDD_MAX_UV);
		regulator_set_voltage(data->vdd, 0, GMC306_VDD_MAX_UV);
#endif    
reg_vdd_put:
	regulator_put(data->vdd);
	return rc;
...
//	if (!gpio_is_valid(akm->gpio_rstn)) {
	if (gpio_is_valid(akm->gpio_rstn)) {
		...
		dev_err(dev, "gpio reset pin %d to be invalid & as expected .\n",
//			akm->gpio_rstn);
			akm->gpio_rstn);
		akm->gpio_rstn = 0;	
		dev_err(dev, "gpio reset pin setted to be %d .\n",
			akm->gpio_rstn);
		//return -EINVAL;
	}
...
//	akm->pin_default = pinctrl_lookup_state(akm->pinctrl, "default");
	akm->pin_default = pinctrl_lookup_state(akm->pinctrl, "gmc306_default");
...
//	akm->pin_sleep = pinctrl_lookup_state(akm->pinctrl, "sleep");
	akm->pin_sleep = pinctrl_lookup_state(akm->pinctrl, "gmc306_sleep");
...
//	int hdata[AKM09911_AXIS_COUNT];
	...
	dev_warn(&akm->i2c->dev,
				"akm_self_test enabled=%d\n",
				akm->state.power_on);
	mutex_lock(&akm->op_mutex);

//	asax = akm->sense_conf[AKM09911_AXIS_X];
	asax = akm->sense_conf[GMC306_AXIS_X];
//	asay = akm->sense_conf[AKM09911_AXIS_Y];
	asay = akm->sense_conf[GMC306_AXIS_Y];
//	asaz = akm->sense_conf[AKM09911_AXIS_Z];
	asaz = akm->sense_conf[GMC306_AXIS_Z];
...
//	case_test(akm, TLIMIT_TN_ASAX_09911, asax, TLIMIT_LO_ASAX_09911,
	case_test(akm, TLIMIT_TN_ASAX_306, asax, TLIMIT_LO_ASAX_306,
//			TLIMIT_HI_ASAX_09911, &fail_total);
			TLIMIT_HI_ASAX_306, &fail_total);
//	case_test(akm, TLIMIT_TN_ASAY_09911, asay, TLIMIT_LO_ASAY_09911,
	case_test(akm, TLIMIT_TN_ASAY_306, asay, TLIMIT_LO_ASAY_306,
//			TLIMIT_HI_ASAY_09911, &fail_total);
			TLIMIT_HI_ASAY_306, &fail_total);
//	case_test(akm, TLIMIT_TN_ASAZ_09911, asaz, TLIMIT_LO_ASAZ_09911,
	case_test(akm, TLIMIT_TN_ASAZ_306, asaz, TLIMIT_LO_ASAZ_306,
//			TLIMIT_HI_ASAZ_09911, &fail_total);
			TLIMIT_HI_ASAZ_306, &fail_total);

//	ret = AKECS_SetMode(akm, AK09911_MODE_SNG_MEASURE);
	ret = AKECS_SetMode(akm, GMC306_MODE_SNG_MEASURE);
	if (ret < 0) {
...
//	hdata[AKM09911_AXIS_X] = (s16)(i2c_data[1] | (i2c_data[2] << 8));
	hdata[GMC306_AXIS_X] = (s16)(i2c_data[1] | (i2c_data[2] << 8));
//	hdata[AKM09911_AXIS_Y] = (s16)(i2c_data[3] | (i2c_data[4] << 8));
	hdata[GMC306_AXIS_Y] = (s16)(i2c_data[3] | (i2c_data[4] << 8));
//	hdata[AKM09911_AXIS_Z] = (s16)(i2c_data[5] | (i2c_data[6] << 8));
	hdata[GMC306_AXIS_Z] = (s16)(i2c_data[5] | (i2c_data[6] << 8));

	i2c_data[0] &= 0x7F;
//	case_test(akm, TLIMIT_TN_SNG_ST1_09911,
	case_test(akm, TLIMIT_TN_SNG_ST1_306,
//	       (int)i2c_data[0], TLIMIT_LO_SNG_ST1_09911,
	       (int)i2c_data[0], TLIMIT_LO_SNG_ST1_306,
//		TLIMIT_HI_SNG_ST1_09911, &fail_total);

		TLIMIT_HI_SNG_ST1_306, &fail_total);

//	case_test(akm, TLIMIT_TN_SNG_HX_09911, hdata[0], TLIMIT_LO_SNG_HX_09911,
	case_test(akm, TLIMIT_TN_SNG_HX_306, hdata[0], TLIMIT_LO_SNG_HX_306,
//			TLIMIT_HI_SNG_HX_09911, &fail_total);
			TLIMIT_HI_SNG_HX_306, &fail_total);
//	case_test(akm, TLIMIT_TN_SNG_HY_09911, hdata[1], TLIMIT_LO_SNG_HY_09911,
	case_test(akm, TLIMIT_TN_SNG_HY_306, hdata[1], TLIMIT_LO_SNG_HY_306,
//			TLIMIT_HI_SNG_HY_09911, &fail_total);
			TLIMIT_HI_SNG_HY_306, &fail_total);
//	case_test(akm, TLIMIT_TN_SNG_HZ_09911, hdata[2], TLIMIT_LO_SNG_HZ_09911,
	case_test(akm, TLIMIT_TN_SNG_HZ_306, hdata[2], TLIMIT_LO_SNG_HZ_306,
//			TLIMIT_HI_SNG_HZ_09911, &fail_total);
			TLIMIT_HI_SNG_HZ_306, &fail_total);

//	case_test(akm, TLIMIT_TN_SNG_ST2_09911, (int)i2c_data[8],
	case_test(akm, TLIMIT_TN_SNG_ST2_306, (int)i2c_data[8],
//			TLIMIT_LO_SNG_ST2_09911, TLIMIT_HI_SNG_ST2_09911,
			TLIMIT_LO_SNG_ST2_306, TLIMIT_HI_SNG_ST2_306,
			&fail_total);

	/* self-test mode */
//	ret = AKECS_SetMode(akm, AK09911_MODE_SELF_TEST);
	ret = AKECS_SetMode(akm, GMC306_MODE_SELF_TEST);
	if (ret < 0) {
...
//	case_test(akm, TLIMIT_TN_SLF_ST1_09911, (int)i2c_data[0],
	case_test(akm, TLIMIT_TN_SLF_ST1_306, (int)i2c_data[0],
//			TLIMIT_LO_SLF_ST1_09911, TLIMIT_HI_SLF_ST1_09911,
			TLIMIT_LO_SLF_ST1_306, TLIMIT_HI_SLF_ST1_306,
			&fail_total);
...
//	case_test(akm, TLIMIT_TN_SLF_RVHX_09911, (hdata[0])*(asax/128 + 1),
	case_test(akm, TLIMIT_TN_SLF_RVHX_306, (hdata[0])*(asax/128 + 1),
//			TLIMIT_LO_SLF_RVHX_09911, TLIMIT_HI_SLF_RVHX_09911,
			TLIMIT_LO_SLF_RVHX_306, TLIMIT_HI_SLF_RVHX_306,
			&fail_total);

//	case_test(akm, TLIMIT_TN_SLF_RVHY_09911, (hdata[1])*(asay/128 + 1),
	case_test(akm, TLIMIT_TN_SLF_RVHY_306, (hdata[1])*(asay/128 + 1),
//			TLIMIT_LO_SLF_RVHY_09911, TLIMIT_HI_SLF_RVHY_09911,
			TLIMIT_LO_SLF_RVHY_306, TLIMIT_HI_SLF_RVHY_306,
			&fail_total);

//	case_test(akm, TLIMIT_TN_SLF_RVHZ_09911, (hdata[2])*(asaz/128 + 1),
	case_test(akm, TLIMIT_TN_SLF_RVHZ_306, (hdata[2])*(asaz/128 + 1),
//			TLIMIT_LO_SLF_RVHZ_09911, TLIMIT_HI_SLF_RVHZ_09911,
			TLIMIT_LO_SLF_RVHZ_306, TLIMIT_HI_SLF_RVHZ_306,
			&fail_total);

//	case_test(akm, TLIMIT_TN_SLF_ST2_09911, (int)i2c_data[8],
	case_test(akm, TLIMIT_TN_SLF_ST2_306, (int)i2c_data[8],
//			TLIMIT_LO_SLF_ST2_09911, TLIMIT_HI_SLF_ST2_09911,
			TLIMIT_LO_SLF_ST2_306, TLIMIT_HI_SLF_ST2_306,
			&fail_total);
...
//	struct akm09911_platform_data *pdata;
	struct gmc306_platform_data *pdata;
	int err = 0;
	int i;

	dev_dbg(&client->dev, "start probing.");
	dev_warn(&client->dev, "akm_compass_probe enter\n");
...
//	err = akm09911_i2c_check_device(client);
	err = gmc306_i2c_check_device(client);
	if (err < 0)
		goto exit4;
...
//	if (s_akm->irq) {
	if (s_akm->irq) {
		dev_info(&client->dev, "s_akm->irq = %d", s_akm->irq);
...
//	} else if (s_akm->auto_report) {
		...
		dev_info(&client->dev, "s_akm->auto_report s_akm->irq = %d", s_akm->irq);
//		if (s_akm->use_hrtimer) {
		if (s_akm->use_hrtimer) {
			dev_info(&client->dev, "s_akm->use_hrtimer s_akm->irq = %d", s_akm->irq);
...
//			INIT_WORK(&s_akm->dwork.work, akm_dev_poll);
			...
			dev_info(&client->dev, "not s_akm->use_hrtimer s_akm->irq = %d", s_akm->irq);
//			s_akm->work_queue = alloc_workqueue("akm_poll_work",
			s_akm->work_queue = alloc_workqueue("akm_poll_work",
				WQ_FREEZABLE, 0);
...
//	err = sensors_classdev_register(&s_akm->input->dev, &s_akm->cdev);
	...
	//20181228 Frank Cheng add to fit layout value backward compatible to DVT&EVT
	gpio_direction_input(97);
	gpio_direction_input(98);
	dev_err(&client->dev, "akm->layout = %d\n", s_akm->layout);
	dev_err(&client->dev, "gpio_get_value(97) = %d\n", gpio_get_value(97));
	dev_err(&client->dev, "gpio_get_value(98) = %d\n", gpio_get_value(98));
	dev_err(&client->dev, "hardware_version = %d\n", hardware_version);
	if((s_akm->layout == 8) && (0 ==  gpio_get_value(97))){
	//if((s_akm->layout == 8) && (hardware_version<2)){
		s_akm->layout = 6;
	}//else{
		//dev_err(&client->dev, "gpio_get_value(97) 1\n");
	//}
	//20181228 Frank Cheng add to fit layout value backward compatible to DVT&EVT
...
//	struct akm_compass_data *akm = i2c_get_clientdata(client);
	struct akm_compass_data *akm = i2c_get_clientdata(client);
	dev_warn(&client->dev, "akm_compass_remove enter\n");
//	if (akm->auto_report) {
	if (akm->auto_report) {
//		if (akm->use_hrtimer) {
		if (akm->use_hrtimer) {
...
static struct of_device_id gmc306_match_table[] = {
//	{ .compatible = "ak,ak09911", },
//	{ .compatible = "akm,akm09911", },
//	{ .compatible = "gmc,gmc306", },
	{ .compatible = "ak,gmc306", },
	{ },
};
...
//		.of_match_table = akm09911_match_table,
		.of_match_table = gmc306_match_table,
...
static int __init akm_compass_init(void)
{
	pr_err("gpio_request(97, ""GPIO97"") = %d\n", gpio_request(97, "GPIO97"));
	pr_err("gpio_request(98, ""GPIO98"") = %d\n", gpio_request(98, "GPIO98"));
