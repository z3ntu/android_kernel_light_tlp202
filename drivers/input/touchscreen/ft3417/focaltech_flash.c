/*
 *
 * FocalTech ft3417 TouchScreen driver.
 *
...
* Static variables
*****************************************************************************/
/*[Arima_4101][allen_yu] Implement the touch firmware upgrade rule by app.bin file 20190308 begin*/
#include <linux/firmware.h>
#include <linux/types.h>
#include <linux/unistd.h>
#include <linux/fcntl.h>
#include <linux/uaccess.h>
/*[Arima_4101][allen_yu] ft3417 upgrade fw 20190318 begin*/
#define FW_FILE_NAME "FT3417_LightPhone_V51_20190318_app.bin"
/*[Arima_4101][allen_yu] 20190318 end*/
const struct firmware *touch_fw = NULL;
#define FTS_FW_REQUEST_SUPPORT                      1
/*[Arima_4101][allen_yu] 20190308 end*/
/*****************************************************************************
...
struct upgrade_fw ft3417_fw_list[] = {
    {FTS_VENDOR_ID, ft3417_fw_file1, sizeof(ft3417_fw_file1)},
    {FTS_VENDOR_ID2, ft3417_fw_file2, sizeof(ft3417_fw_file2)},
    {FTS_VENDOR_ID3, ft3417_fw_file3, sizeof(ft3417_fw_file3)},
};

struct upgrade_func *ft3417_upgrade_func_list[] = {
    &upgrade_func_ft5x46,
};
struct ft3417_upgrade *ft3417_fwupgrade;
extern int FT3417_NOT_TP;

/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static u16 ft3417_pram_ecc_calc_host(u8 *pbuf, u16 length)
{
    u16 ecc = 0;
    u16 i = 0;
    u16 j = 0;
...
/************************************************************************
 * ft3417_pram_ecc_cal - Calculate and get pramboot ecc
 *
 * return pramboot ecc of tp if success, otherwise return error code
 ***********************************************************************/
static int ft3417_pram_ecc_cal_algo(
    ...
    u32 ecc_length)
{
    ...
    ret = ft3417_i2c_write(client, cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < 100; i++) {
        msleep(1);
        ret = ft3417_i2c_read(client, cmd, 1, val, 1);
        if (ret < 0) {
    ...
    ret = ft3417_i2c_read(client, cmd, 1, val, 2);
    if (ret < 0) {
...
static int ft3417_pram_ecc_cal_xor(struct i2c_client *client)
{
    int ret = 0;
    u8 reg_val = 0;
    ...
    ret = ft3417_i2c_read_reg(client, FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
static int ft3417_pram_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
    if ((NULL == ft3417_fwupgrade) && (NULL == ft3417_fwupgrade->func)) {
        FTS_ERROR("ft3417_fwupgrade/func is null");
        return -EINVAL;
    }

    if (ft3417_fwupgrade->func->newmode) {
        return ft3417_pram_ecc_cal_algo(client, saddr, len);
    } else {
        return ft3417_pram_ecc_cal_xor(client);
    }
}

/************************************************************************
 * ft3417_pram_write_buf - write pramboot data and calculate ecc
 *
 * return pramboot ecc of host if success, otherwise return error code
 ***********************************************************************/
static int ft3417_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 j = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = ft3417_i2c_write(client, packet_buf, packet_len + FTS_CMD_WRITE_LEN);
        if (ret < 0) {
    ...
    if (ft3417_fwupgrade->func->newmode) {
        ecc_in_host = (int)ft3417_pram_ecc_calc_host(buf, len);
    } else {
...
/************************************************************************
 * ft3417_pram_start - remap to start pramboot
 *
 * return 0 if success, otherwise return error code
 ***********************************************************************/
static int ft3417_pram_start(struct i2c_client *client)
{
    ...
    ret = ft3417_i2c_write(client, &cmd, 1);
    if (ret < 0) {
...
/************************************************************************
 * ft3417_pram_write_remap - write pramboot to pram and start pramboot
 *
 * return 0 if success, otherwise return error code
***********************************************************************/
static int ft3417_pram_write_remap(struct i2c_client *client)
{
    int ret = 0;
    int ecc_in_host = 0;
    int ecc_in_tp = 0;
    u8 *pb_buf = NULL;
    u32 pb_len = 0;
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
    ...
    ecc_in_host = ft3417_pram_write_buf(client, pb_buf, pb_len);
    if (ecc_in_host < 0) {
    ...
    /* read out checksum */
    ecc_in_tp = ft3417_pram_ecc_cal(client, 0, pb_len);
    if (ecc_in_tp < 0) {
        FTS_ERROR( "read pramboot ecc fail");
        return ecc_in_tp;
    }
...
/************************************************************************
 * ft3417_pram_init - initialize pramboot
 *
 * return 0 if success, otherwise return error code
***********************************************************************/
static int ft3417_pram_init(struct i2c_client *client)
{
    int ret = 0;
    u8 reg_val = 0;
    u8 wbuf[3] = { 0 };
    ...
    ret = ft3417_i2c_read(client, wbuf, 1, &reg_val, 1);
    if (ret < 0) {
    ...
    wbuf[1] = reg_val;
    wbuf[2] = 0x00;
    ret = ft3417_i2c_write(client, wbuf, 3);
    if (ret < 0) {
...
/************************************************************************
* Name: ft3417_pram_write_init
* Brief: wirte pramboot to pram and initialize
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_pram_write_init(struct i2c_client *client)
{
    int ret = 0;
    bool state = 0;
    ...
    if (status != FTS_RUN_IN_ROM) {
        ...
        ret = ft3417_fwupg_reset_to_romboot(client);
        if (ret < 0) {
    ...
    ret = ft3417_pram_write_remap(client);
    if (ret < 0) {
    ...
    state = ft3417_fwupg_check_state(client, FTS_RUN_IN_PRAM);
    if (!state) {
...
/************************************************************************
* Name: ft3417_fwupg_check_fw_valid
* Brief: check fw in tp is valid or not
* Input:
* Output:
* Return: return true if fw is valid, otherwise return false
***********************************************************************/
bool ft3417_fwupg_check_fw_valid(struct i2c_client *client)
{
    int ret = 0;

    ret = ft3417_wait_tp_to_valid(client);
    if (ret < 0) {
        FTS_INFO("FT3417 tp fw invaild");
//        return false;
    }

    FTS_INFO("FT3417 tp fw vaild");
    return true;
}

/************************************************************************
* Name: ft3417_fwupg_get_boot_state
* Brief: read boot id(rom/pram/bootloader), confirm boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
    int ret = 0;
    u8 cmd[4] = { 0 };
    u32 cmd_len = 0;
    u8 val[2] = { 0 };
    struct ft_chip_t ids = ft3417_data->ic_info.ids;
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
    ...
    if (upg->func->hid_supported)
        ft3417_i2c_hid2std(client);

    cmd[0] = FTS_CMD_START1;
    cmd[1] = FTS_CMD_START2;
    ret = ft3417_i2c_write(client, cmd, 2);
    if (ret < 0) {
    ...
    if (ft3417_data->ic_info.is_incell)
        cmd_len = FTS_CMD_READ_ID_LEN_INCELL;
    else
        cmd_len = FTS_CMD_READ_ID_LEN;
    ret = ft3417_i2c_read(client, cmd, cmd_len, val, 2);
    if (ret < 0) {
...
bool ft3417_fwupg_check_state(struct i2c_client *client, enum FW_STATUS rstate)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = ft3417_fwupg_get_boot_state(client, &cstate);
...
/************************************************************************
* Name: ft3417_fwupg_reset_in_boot
* Brief: RST CMD(07), reset to romboot(bootloader) in boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_fwupg_reset_in_boot(struct i2c_client *client)
{
    int ret = 0;
...
/************************************************************************
* Name: ft3417_fwupg_reset_to_boot
* Brief: reset to boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_fwupg_reset_to_boot(struct i2c_client *client)
{
    int ret = 0;

    FTS_INFO("send 0xAA and 0x55 to FW, reset to boot environment");

    ret = ft3417_i2c_write_reg(client, FTS_REG_UPGRADE, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = ft3417_i2c_write_reg(client, FTS_REG_UPGRADE, FTS_UPGRADE_55);
    if (ret < 0) {
...
/************************************************************************
* Name: ft3417_fwupg_reset_to_romboot
* Brief: reset to romboot, to load pramboot
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_fwupg_reset_to_romboot(struct i2c_client *client)
{
    int ret = 0;
    int i = 0;
    u8 cmd = FTS_CMD_RESET;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = ft3417_fwupg_get_boot_state(client, &state);
...
/************************************************************************
* Name: ft3417_fwupg_enter_into_boot
* Brief: enter into boot environment, ready for upgrade
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_fwupg_enter_into_boot(struct i2c_client *client)
{
    int ret = 0;
    bool fwvalid = false;
    bool state = false;
    ...
    fwvalid = ft3417_fwupg_check_fw_valid(client);
    if (fwvalid) {
        ret = ft3417_fwupg_reset_to_boot(client);
        if (ret < 0) {
    ...
    if (upg->func->pramboot_supported) {
        ...
        /* pramboot */
        ret = ft3417_pram_write_init(client);
        if (ret < 0) {
        ...
        /* bootloader */
        state = ft3417_fwupg_check_state(client, FTS_RUN_IN_BOOTLOADER);
        if (!state) {
...
/************************************************************************
 * Name: ft3417_fwupg_check_flash_status
...
***********************************************************************/
static bool ft3417_fwupg_check_flash_status(
    struct i2c_client *client,
    u16 flash_status,
    int retries,
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = ft3417_i2c_read(client, &cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
...
/************************************************************************
 * Name: ft3417_fwupg_erase
 * Brief: erase flash area
 * Input: delay - delay after erase
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ft3417_fwupg_erase(struct i2c_client *client, u32 delay)
{
    int ret = 0;
    u8 cmd = 0;
    bool flag = false;
    ...
    flag = ft3417_fwupg_check_flash_status(client, FTS_CMD_FLASH_STATUS_ERASE_OK,
 ...
 ***********************************************************************/
int ft3417_fwupg_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
    int ret = 0;
    u32 i = 0;
    ...
    ret = ft3417_i2c_write(client, wbuf, 1);
    if (ret < 0) {
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = ft3417_i2c_write(client, wbuf, FTS_CMD_ECC_CAL_LEN);
        if (ret < 0) {
        ...
        ret = ft3417_fwupg_check_flash_status(client, FTS_CMD_FLASH_STATUS_ECC_OK,
    ...
    ret = ft3417_i2c_read(client, wbuf, 1, val, 1);
    if (ret < 0) {
 ...
 ***********************************************************************/
int ft3417_flash_read_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 packet_number = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = ft3417_i2c_write(client, wbuf, FTS_CMD_READ_LEN);
        if (ret < 0) {
        ...
        ret = ft3417_i2c_read(client, NULL, 0, buf + offset, packet_len);
        if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ft3417_flash_read(struct i2c_client *client, u32 addr, u8 *buf, u32 len)
{
    int ret = 0;
    ...
    ret = ft3417_fwupg_enter_into_boot(client);
    if (ret < 0) {
        FTS_ERROR("enter into pramboot/bootloader fail");
        goto read_flash_err;
    }

    ret = ft3417_flash_read_buf(client, addr, buf, len);
    if (ret < 0) {
...
read_flash_err:
    ...
    ret = ft3417_fwupg_reset_in_boot(client);
    if (ret < 0) {
...
/************************************************************************
 * Name: ft3417_read_file
 * Brief:  read file
 * Input: file name
 * Output:
 * Return: return file len if succuss, otherwise return error code
 ***********************************************************************/
int ft3417_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
...
/************************************************************************
* Name: ft3417_upgrade_bin
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3417_upgrade_bin(struct i2c_client *client, char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
    ...
    ret = ft3417_read_file(fw_name, &fw_file_buf);
    ...
    if (ret < 0) {
        ...
        ft3417_fwupg_reset_in_boot(client);
        goto err_bin;
    }
...
#if FTS_AUTO_LIC_UPGRADE_EN
static int ft3417_lic_get_vid_in_tp(struct i2c_client *client, u16 *vid)
{
    int ret = 0;
    u8 val[2] = { 0 };
    ...
    ret = ft3417_i2c_read_reg(client, FTS_REG_VENDOR_ID, &val[0]);
    if (ft3417_data->ic_info.is_incell)
        ret = ft3417_i2c_read_reg(client, FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int ft3417_lic_get_vid_in_host(u16 *vid)
{
    u8 val[2] = { 0 };
    u8 *licbuf = NULL;
    u32 conf_saddr = 0;
...
static int ft3417_lic_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    int ret = 0;
    ...
    ret = ft3417_i2c_read_reg(client, FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int ft3417_lic_get_ver_in_host(u8 *ver)
{
    int ret = 0;
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
...
*/
static bool ft3417_lic_need_upgrade(struct i2c_client *client)
{
    int ret = 0;
    ...
    ret = ft3417_lic_get_vid_in_host(&vid_in_host);
    if (ret < 0) {
        FTS_ERROR("vendor id in host invalid");
        return false;
    }

    ret = ft3417_lic_get_vid_in_tp(client, &vid_in_tp);
    if (ret < 0) {
    ...
    ret = ft3417_lic_get_ver_in_host(&initcode_ver_in_host);
    if (ret < 0) {
        FTS_ERROR("init code in host invalid");
        return false;
    }

    ret = ft3417_lic_get_ver_in_tp(client, &initcode_ver_in_tp);
    if (ret < 0) {
...
int ft3417_lic_upgrade(struct i2c_client *client, struct ft3417_upgrade *upg)
{
    int ret = 0;
    ...
    hlic_upgrade = ft3417_lic_need_upgrade(client);
    ...
    if (hlic_upgrade) {
        ...
        do {
            ...
            if (ret < 0) {
                ...
                ft3417_lic_get_ver_in_tp(client, &ver);
...
static int ft3417_param_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    int ret = 0;
    ...
    ret = ft3417_i2c_read_reg(client, FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static int ft3417_param_get_ver_in_host(u8 *ver)
{
    struct ft3417_upgrade *upg = ft3417_fwupgrade;

    if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
        FTS_ERROR("ft3417_data/upgrade/func/fw/ver is NULL");
        return -EINVAL;
    }
...
/************************************************************************
 * ft3417_param_need_upgrade - check fw paramcfg need upgrade or not
 *
 * Return: return true if paramcfg need upgrade
 ***********************************************************************/
static bool ft3417_param_need_upgrade(struct i2c_client *client)
{
    int ret = 0;
    u8 val = 0;
    u8 ver_in_host = 0;
    ...
    ret = ft3417_param_get_ver_in_host(&ver_in_host);
    if (ret < 0) {
        FTS_ERROR("param version in host invalid");
        return false;
    }

    ret = ft3417_i2c_read_reg(client, FTS_REG_IDE_PARA_STATUS, &val);
    if (ret < 0) {
    ...
    ret = ft3417_param_get_ver_in_tp(client, &ver_in_tp);
    if (ret < 0) {
...
/************************************************************************
 * ft3417_fwupg_get_ver_in_tp - read fw ver from tp register
 *
 * return 0 if success, otherwise return error code
 ***********************************************************************/
static int ft3417_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    int ret = 0;
    ...
    ret = ft3417_i2c_read_reg(client, FTS_REG_FW_VER, ver);
    if (ret < 0) {
...
/************************************************************************
 * ft3417_fwupg_get_ver_in_host - read fw ver in host fw image
 *
 * return 0 if success, otherwise return error code
 ***********************************************************************/
static int ft3417_fwupg_get_ver_in_host(u8 *ver)
{
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
...
/************************************************************************
 * ft3417_fwupg_need_upgrade - check fw need upgrade or not
 *
 * Return: return true if fw need upgrade
 ***********************************************************************/
static bool ft3417_fwupg_need_upgrade(struct i2c_client *client)
{
    int ret = 0;
    bool fwvalid = false;
    u8 fw_ver_in_host = 0;
    ...
    fwvalid = ft3417_fwupg_check_fw_valid(client);
    if (fwvalid) {
		printk("FT3417_NOT_TP = %d\n",FT3417_NOT_TP);
		if(!FT3417_NOT_TP){
        ret = ft3417_fwupg_get_ver_in_host(&fw_ver_in_host);
        if (ret < 0) {
            FTS_ERROR("get fw ver in host fail");
            return false;
        }

        ret = ft3417_fwupg_get_ver_in_tp(client, &fw_ver_in_tp);
        if (ret < 0) {
...
/************************************************************************
 * Name: ft3417_fw_upgrade
 * Brief: fw upgrade main entry
 * Input:
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ft3417_fwupg_upgrade(struct i2c_client *client, struct ft3417_upgrade *upg)
{
    int ret = 0;
    ...
    upgrade_flag = ft3417_fwupg_need_upgrade(client);
        ...
        if (upgrade_flag) {
            ...
            if (upg->func->upgrade) {
                ...
                if (ret < 0) {
                    ...
                    ft3417_fwupg_get_ver_in_tp(client, &ver);
...
/************************************************************************
 * ft3417_fwupg_auto_upgrade - upgrade main entry
 ***********************************************************************/
void ft3417_fwupg_auto_upgrade(struct ft3417_ts_data *ts_data)
{
    int ret = 0;
    ...
    ret = ft3417_fwupg_upgrade(client, upg);
    if (ret < 0)
...
#if FTS_AUTO_LIC_UPGRADE_EN
    ret = ft3417_lic_upgrade(client, upg);
    if (ret < 0)
...
/*
 * ft3417_fwupg_work - fw upgrade work function, handle upgrade
 * @vid - u16 consit of module id and panel id
 *
 * return 0 if success, otherwise return error code
 */
int ft3417_fwupg_get_vendorid(struct ft3417_ts_data *ts_data, u16 *vid)
{
    int ret = 0;
    bool fwvalid = false;
    u8 vendor_id = 0;
    ...
    fwvalid = ft3417_fwupg_check_fw_valid(client);
    if (fwvalid) {
        ret = ft3417_i2c_read_reg(client, FTS_REG_VENDOR_ID, &vendor_id);
        if (ts_data->ic_info.is_incell)
            ret = ft3417_i2c_read_reg(client, FTS_REG_MODULE_ID, &module_id);
    } else {
        fwcfg_addr =  upg->func->fwcfgoff;
        ret = ft3417_flash_read(client, fwcfg_addr, cfgbuf, FTS_HEADER_LEN);
...
/*
 * ft3417_fwupg_get_fw_file - get upgrade fw file in host driver
 *
 * return 0 if success, otherwise return error code
 *
 ...
 */
static int ft3417_fwupg_get_fw_file(struct ft3417_ts_data *ts_data)
{
    struct upgrade_fw *fw = &ft3417_fw_list[0];
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
/*[Arima_4101][allen_yu] Implement the touch firmware upgrade rule by app.bin file 20190308 begin*/
    u8 *tmpbuf = NULL;
    int ret = 0;
/*[Arima_4101][allen_yu] 20190308 end*/
    ...
    ret = ft3417_fwupg_get_vendorid(ts_data, &vendor_id);
    if (ret < 0) {
...
/*[Arima_4101][allen_yu] Implement the touch firmware upgrade rule by app.bin file 20190308 begin*/
    if (upg) {
    	if(FTS_FW_REQUEST_SUPPORT){
		        ret = request_firmware(&touch_fw,FW_FILE_NAME,ts_data->dev);
				FTS_INFO("request_firmware_retvalue : %d(%s)(%d)\n" , ret, __func__, __LINE__ );

				if (ret < 0) {
						FTS_INFO("Request firmware failed - %s (%d)\n",
								FW_FILE_NAME, ret);
						return ret;
				}else{
						tmpbuf = vmalloc(touch_fw->size);
						memcpy(tmpbuf, touch_fw->data, touch_fw->size);
						upg->fw = tmpbuf;
						upg->fw_length = touch_fw->size;
						FTS_INFO("request_firmware size : %zu(%s)(%d)\n" , touch_fw->size, __func__, __LINE__ );
					    FTS_INFO("request_firmware data: %02X, %02X, %02X \n",
								touch_fw->data[0],touch_fw->data[1],touch_fw->data[2]);
				}
...
/*
 * ft3417_fwupg_init_ic_detail - for ic detail initialaztion
 */
static void ft3417_fwupg_init_ic_detail(void)
{
    struct ft3417_upgrade *upg = ft3417_fwupgrade;
...
/*
 * ft3417_fwupg_work - fw upgrade work function
 * 1. get fw image/file
 * 2. call upgrade main function(ft3417_fwupg_auto_upgrade)
 */
static void ft3417_fwupg_work(struct work_struct *work)
{
    int ret = 0;
    struct ft3417_ts_data *ts_data = container_of(work,
                                  struct ft3417_ts_data, fwupg_work);
    ...
    ret = ft3417_fwupg_get_fw_file(ts_data);
    ft3417_fwupg_init_ic_detail();
    if (ret < 0) {
        FTS_ERROR("get file fail, can't upgrade");
    } else {
        /* run auto upgrade */
        ft3417_fwupg_auto_upgrade(ts_data);
    }
...
/*****************************************************************************
 *  Name: ft3417_fwupg_init
 *  Brief: upgrade function initialization
 *  Input:
 *  Output:
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
int ft3417_fwupg_init(struct ft3417_ts_data *ts_data)
{
    int i = 0;
    int j = 0;
    int ic_stype = ts_data->ic_info.ids.type;
    struct upgrade_func *func = ft3417_upgrade_func_list[0];
    int func_count = sizeof(ft3417_upgrade_func_list) / sizeof(ft3417_upgrade_func_list[0]);

    FTS_INFO("fw upgrade init function");

    if (NULL == ts_data->ts_workqueue) {
        FTS_ERROR("ft3417 workqueue is NULL, can't run upgrade function");
        return -EINVAL;
    }
    ...
    ft3417_fwupgrade = (struct ft3417_upgrade *)kzalloc(sizeof(*ft3417_fwupgrade), GFP_KERNEL);
    if (NULL == ft3417_fwupgrade) {
        FTS_ERROR("malloc memory for upgrade fail");
        return -ENOMEM;
    }

    if (1 == func_count) {
        ft3417_fwupgrade->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = ft3417_upgrade_func_list[i];
    ...
    if (NULL == ft3417_fwupgrade->func) {
        FTS_ERROR("no upgrade functin match, can't upgrade");
        return -ENODATA;
    }

    INIT_WORK(&ts_data->fwupg_work, ft3417_fwupg_work);
...
/*****************************************************************************
 *  Name: ft3417_fwupg_exit
 *  Brief:
 *  Input:
 *  Output:
 *  Return:
 *****************************************************************************/
int ft3417_fwupg_exit(struct ft3417_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (ft3417_fwupgrade) {
        kfree(ft3417_fwupgrade);
    }
    FTS_FUNC_EXIT();
